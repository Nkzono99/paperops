...
| `template/_paperops/model/` | project editorial model | P2 project-owned per-ID typed state | adapt | human / skills | model ごとの単一 CLI or approved workflow | preflight、atomic write、post-validation、rollback | 全 project migration と復元検証後のみ |
...
| `template/.agents/skills/note-writing-session/SKILL.md` | session handoff 記録 | P1–P6 operational note | investigate | Agent / human | 未確定: P1 で session note と authority fact の境界を調査 | authority と誤認しない印を付ける | 調査後の後継導線と利用ゼロ確認後のみ |
