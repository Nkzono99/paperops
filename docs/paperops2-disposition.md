...
| `template/_paperops/model/` | project editorial model | P2 project-owned per-ID typed state | investigate | human / skills | 未確定: model ごとの単一 writer | 理由: modelごとに単一writerをP1で決定する必要がある。決定後に preflight、atomic write、post-validation、rollback を設計する | writer 決定、全 project migration、復元検証後のみ |
