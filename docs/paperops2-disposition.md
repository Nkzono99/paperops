...
P1 は schema・canonical hash・migration 基盤、P2 はmodel migration、P3はcompiler / Writer、P4はtyped workflow、P7は新規projectの既定化を指す。legacy removalは利用観測、互換reader利用ゼロ、復元検証を満たした個別変更に分離する。既存projectでは legacy-authoritative → shadow → opt-in v2-authoritative のgateを引き続き順守する。
...
| `template/_paperops/model/` | project-owned Research / Editorial / Results hierarchy / Manuscript / Issue / Publication typed state | P1 schema、P2 migration、P7 new-project authority | adapt | 既存projectはlegacyまたは明示採用済みmodel、新規projectはtyped model | model / workflow CLIのtransaction writer | 新規initは六モデルを検証してhashを固定する。managed updateはindex/recordや既存authorityを生成・上書きせず、legacy artifactへdual-writeしない | 互換reader利用ゼロ、複数実projectの復元確認、別の削除判断後のみ |
| `template/_paperops/claims/` | claim / argument / gate cards | Research / Editorial | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | existing project-owned pathはupdateで削除せずread-only adapterを保持 | new scaffoldでは削除済み |
| `template/_paperops/evidence/` | result / figure / source evidence cards | Research | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | provenanceをtyped candidateへ保存的変換 | new scaffoldでは削除済み |
| `template/_paperops/evidence/figures/` | figure evidence records | Research figure | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | figure IDと参照をmigration | new scaffoldでは削除済み |
| `template/_paperops/evidence/results/` | result evidence records | Research result | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | predictedとobservedを混同しない | new scaffoldでは削除済み |
| `template/_paperops/evidence/sources/` | source evidence records | Research source | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | raw/private sourceはlocal stateに残す | new scaffoldでは削除済み |
...
| `template/_paperops/review/` | review rounds、feedback、responses | Issue | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | raw reviewer textをlocal/confidential stateと分離しdual-writeしない | new scaffoldでは削除済み |
| `template/_paperops/requests/` | analysis / writing request lifecycle | Issue | remove from new scaffold | existing project / migration reader only | `pops change` transaction writer | lifecycleを保存的にtyped candidateへ変換 | new scaffoldでは削除済み |
| `template/_paperops/workflow/` | writable macro state / ledger | six-model projection / Publication | remove from new scaffold | existing project / migration reader only | workflow/change transaction writer | directoryはoverlay用に残り得るがcurrent-state等のstarterは削除 | new scaffoldでは削除済み |
