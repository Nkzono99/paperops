...
通常の Editorial state では `results_hierarchy.document` を project root 相対で解決し、そのファイルの RHI ID へ接続する。`--document` fixture override だけを指定した場合は override 文書の親 directory 相対、`--results-document` を指定した場合だけその明示 override を使う。registry に index-backed model がある場合は record path / schema、index の ID / type / revision / hash、duplicate、orphan も検査し、壊れた record から legacy card へ fallback しない。checked-in registry は Editorial / Results hierarchy / Research / Manuscript / Issue の5 entryで、三つの index starter は架空recordを持たない。Manuscript semantics は section membership、block order、compile provenance、Research approval / gate、dependency hash driftを検査する。Issue semantics は `executed` の出力参照、`reconciled` の照合と人間 signoff、`closed` response の closure criteria・open request・明示 scope change approval・human decision を別 code で検査する。`semantic.predicted_unresolved` は Issue 単独では warning とし、公開対象への依存時の error 昇格は後続 Publication checker が担当する。managed update は registry / schema / scriptsだけを更新し、project-owned index/record、TeX、legacy stateを生成・上書き・削除しない。Publication state、全 model cross-reference、dependency target 解決は未提供である。
