...
- `pops detach <managed-path> [path] --reason <reason>` と `pops detach list [path]` を追加し、managed core file を下流 project で明示 fork できるようにした。detached file は `.pops/manifest.toml` の `[detached]` に path、reason、source version、timestamp を記録し、`update-paperops` では `detached managed files` として報告して自動更新候補から外す。
