...
- CLI 主導の配布導線として Python パッケージ `paper-ops` と `pops` entrypoint を追加し、`init`、`setup`、`doctor`、`update-harness`、`migrate`、`feedback`、`version` の最小サブコマンドを実装した。`pops init` は `template/` を bundled scaffold として展開し、`.pops/manifest.toml` を作成する。あわせて GitHub template repository への publish workflow / helper script を廃止し、root の `make smoke` に `cli-smoke` を追加した。既存下流リポジトリで CLI 管理へ寄せる場合は `pops migrate --apply` を実行する。`raise-template-feedback` は `feedback-paper-harness` に改名した。
