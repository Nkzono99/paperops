...
- `pops update-paperops` を管理対象ハーネス更新の主コマンドにし、旧 `pops update-harness` は互換 alias として残した。`pops` は TTY 上の通常実行時に PyPI の `paper-harness-cli` 最新版を低頻度で確認し、更新がある場合は `uvx --from paper-harness-cli pops setup` と `/update-paperops` スキルでの差分確認を案内する。既存下流リポジトリで取り込む場合は、新規 `update-paperops` skill、AGENTS/CLAUDE のスキル一覧、`docs/cli.md` の新コマンド名を反映する。
- `pops init` / `pops setup` が project-local `.venv` を作成し、既定で `paper-harness-cli==<実行中のバージョン>` をその環境にインストールするようにした。以後の `pops` は activate 済み `.venv` の console script を使う。環境構築を分けたい場合は `--skip-venv` / `--skip-install`、別配布元を使う場合は `--install-spec` を指定できる。`pops doctor` は `.venv` と project-local `pops` の不足を warning として報告する。既存下流リポジトリで取り込む場合は `uvx --from paper-harness-cli pops setup` を一度実行し、`template/README.md`、`AGENTS.md`、`CLAUDE.md`、`.claude/skills/setup/` の案内更新を反映する。
...
- CLI 主導の配布導線として Python パッケージ `paper-harness-cli` と `pops` entrypoint を追加し、GitHub リポジトリ名を `paperops` に変更した。PyPI の `paperops` / `paper-ops` 名は既存プロジェクトとの衝突または類似名ブロックに当たるため、distribution 名は `paper-harness-cli`、CLI 名は `pops` とする。`init`、`setup`、`doctor`、`update-paperops`（旧 `update-harness` 互換 alias）、`migrate`、`feedback`、`version` の最小サブコマンドを実装し、`pops init` は `template/` を bundled scaffold として展開して `.pops/manifest.toml` を作成する。あわせて旧 scaffold 公開 workflow / helper script を廃止し、PyPI Trusted Publishing workflow と root の `make smoke` 用 `cli-smoke` を追加した。既存下流リポジトリで CLI 管理へ寄せる場合は `pops setup` を実行し、旧 scaffold 判定だけ確認したい場合は `pops migrate --apply` を使う。`raise-template-feedback` は `feedback-paper-harness` に改名した。
