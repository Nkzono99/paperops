...
- `pops update-paperops` を管理対象ハーネス更新の主コマンドにし、旧 `pops update-harness` は互換 alias として残した。`pops` は TTY 上の通常実行時に PyPI の `paper-harness-cli` 最新版、実行中の `pops` version、`.pops/manifest.toml` に記録された適用済み scaffold version を比較し、更新がある場合は `uvx --from paper-harness-cli pops update-paperops --dry-run` と `/update-paperops` スキルでの差分確認を案内する。既存下流リポジトリで取り込む場合は、新規 `update-paperops` skill、AGENTS/CLAUDE のスキル一覧、`docs/cli.md` の新コマンド名を反映する。
