...
- `check-claim-evidence.py` と `check-submission-drift.py` に `--strict` を追加し、`finish-manuscript-check` では claim-evidence drift、`submission-gate` / `pre-submit` では submission drift を warning のまま通さず error として検出するようにした。既存下流リポジトリで取り込む場合は、該当 script、Makefile、AGENTS / CLAUDE、docs を更新する必要がある。
