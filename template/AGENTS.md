...
- `_paperops/defaults/schemas/registry.yml` と JSON Schema は paperops-managed、`_paperops/model/editorial/editorial-model.yml`、`results-hierarchy.yml`、Research / Manuscript index/records は project-owned の model state であり混同しない。`make schema-check` は schema / references / semantics / hash phases を advisory に検査し、`--phase hash` は hashability を独立確認する。`results_hierarchy.document` は通常 project root 相対、`--document` fixture では文書の親 directory 相対に解決し、明示 `--results-document` だけが上書きする。model 値は JSON 互換型だけを使う。Research / Manuscript starter は架空recordを持たない空indexである。Manuscript record は section / block の構造・参照・compile provenance を保持し、本文 prose は保持しない。TeX と legacy claim/evidence/gate cards の authority はまだ切り替えず、authority 切替前は明示的な `python scripts/check-paperops-models.py --root . --strict` を成功させ、P2 までは legacy controlled view と既存 checker を維持する。P1-B の Issue / Publication Model、全 model cross-reference、dependency target 解決は未提供である。
