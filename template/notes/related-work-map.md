...
広い探索や web search の raw output は `refs/research/` に一時保持し、採用する文献だけ `refs/summaries/`、`manuscript/shared/bib/references.bib`、必要なら `evidence/sources/` と `notes/views/claim-evidence-map.md` へ昇格する。
...
`refs/research/<topic-slug>/outline.toml` に置く調査設計の要約。
...
`refs/summaries/` と `.bib` へ昇格する文献だけを書く。
