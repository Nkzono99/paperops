...
- `_paperops/refs/research/README.md`
- `_paperops/refs/source-reach/README.md`
- `_paperops/refs/links.toml`
- `_paperops/refs/local/locations.example.toml`
- 必要に応じて `manuscript/venue.md`、`_paperops/notes/views/scientific-gate.md`

## Source channels

`_paperops/notes/source-reach.md` の channel table を更新する。

| channel | 例 | 既定の扱い |
| --- | --- | --- |
| `paper-metadata` | DOI、arXiv、出版社ページ | web で確認し、採用時は `.bib` と `_paperops/refs/summaries/` へ昇格 |
...
| `local-link` | 外部 project、runops、データディレクトリ | `_paperops/refs/links.toml` と `_paperops/refs/local/` で解決 |
...
- raw search result、HTML、transcript、SNS dump は `_paperops/refs/source-reach/<topic-slug>/raw/` か `_paperops/refs/research/<topic-slug>/results/` に置き、既定では Git 管理しない。
- 人間が確認した要約だけを `_paperops/notes/source-reach.md`、`_paperops/notes/related-work-map.md`、`_paperops/refs/summaries/`、`_paperops/notes/reproducibility.md` へ昇格する。
...
- 再現性へ使う: `_paperops/notes/reproducibility.md`
...
- `Next routes`
- `Files updated`: 更新した `_paperops/notes/source-reach.md`、`_paperops/refs/source-reach/`、関連 notes / refs
