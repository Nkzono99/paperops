...
`submission/<venue>/round-1/`、`submission/<venue>/round-2/` のように round snapshot を分け、`_paperops/workflow/submission-ledger.yml` に source commit、gate report、submitted artifact、review response を記録する。これは重複ではなく提出物の証跡である。
...
4. `submission-gate` で通った artifact を `_paperops/workflow/submission-ledger.yml` に記録し、`frozen` 以降の round snapshot を編集しない。
...
6. 査読コメントは `_paperops/review/rounds/` と `_paperops/review/feedback/` に入れ、`route-manuscript-feedback` で evidence / story / section / prose / submission loop へ戻す。
