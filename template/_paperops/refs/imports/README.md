...
`_paperops/refs/links.toml` は「どの外部 project / directory を参照するか」を表す。`_paperops/refs/imports/*.toml` は、その link から取り込む bundle がどの provenance state にあるかを表す。
...
1. `import-state-template.toml` を `_paperops/refs/imports/<link-id>-<bundle>.toml` へコピーする。
2. `link_id` は `_paperops/refs/links.toml` の id に合わせる。
...
個人環境の絶対パスはここに書かない。実パスは ignored な `_paperops/refs/local/locations.toml` に置く。
