...
- `_paperops/defaults/schemas/registry.yml` と JSON Schema は paperops-managed、Editorial / Research / Manuscript / Issue の `_paperops/model/` state は project-owned であり混同しない。Research / Manuscript / Issue starter は架空recordを持たない空indexである。Issue recordには公開可能summaryとopaque local-reference IDだけを置き、raw confidential text、絶対path、credentialを保存しない。`executed` analysisは出力参照、`reconciled`は照合とhuman signoff、`closed` responseはclosure criteria、open request、明示scope change approval、human decisionを検査する。未解決predictionはIssue単独ではwarningで、公開依存時のerror昇格は後続Publication checkerが担う。TeXとlegacy cardのauthorityは切り替えず、P2までは既存checkerも維持する。Publication Model、全model cross-reference、dependency target解決は未提供である。
