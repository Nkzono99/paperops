...
`contracts/` は章ごとの文章テンプレートではなく、storyline、section、figure story の入出力契約を置く場所である。

各 contract は、論文全体または各章が読者のどの疑問に答え、どの card / view / profile を入力にし、どの状態まで仕上げるかを定める。文言や段落数は固定しすぎない。

基本の流れ:

1. `design-paper-storyline`: `storyline.yml` と `notes/views/storyline.md` を読み、reader_promise、evidence_ladder、Results hierarchy、Discussion functions を固定する。
