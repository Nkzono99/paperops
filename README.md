...
PaperOps 2 は P1-A の Editorial Model 縦切りに加え、P1-B Task 3 / 4 の Research / Manuscript Model schema、空 starter index、意味検証を提供する。paperops-managed の `registry.yml` / JSON Schema / checker と、project-owned の model state を分離し、schema → references → semantics → canonical semantic-v1 hash の順で検証する。評価 corpus は mechanism-led、boundary-led、negative-result-led の三つの合成 fixture である。Issue / Publication Model、全 model cross-reference、dependency target 解決は未提供である。
